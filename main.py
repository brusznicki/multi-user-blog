...
class PostShow(Handler):
    def get(self, post_id):
        ...
        p = ndb.Key('Post', int(post_id)).get
        ...
        if not p:
            ...
            likes = Like.query(Like.post_key == p.key).fetch()
            like_count = likes.count()
            # like_count = Like.filter(Like.post_key == p.key).count()
            # comments = Comment.query(Comment.post_key == p.key) \
